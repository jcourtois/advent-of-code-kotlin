...
class Day4Part2 {
    ...
    companion object {
        ...
        fun eval(s: String): Pair<Int, Board> {
            ...
            return playLosingGame(callNumbers, boards)
        }

        private fun playLosingGame(startingBoards: List<Board>, numbers: List<Int>): Pair<Int, Board> {
            var boards = startingBoards
            var winners: List<Pair<Int, Board>> = emptyList()
            for(calledNumber in numbers) {
                boards = boards.map { it.update(calledNumber) }
                val (newWinners, stillInGame) = boards.partition { it.hasWon }
                boards = stillInGame
                winners = winners + (Array(newWinners.size) { calledNumber} zip newWinners)
            }
            return winners.last()
        }

        private fun parse(s: String): Pair<List<Board>, List<Int>> {
            val split = s.split("\n\n")
