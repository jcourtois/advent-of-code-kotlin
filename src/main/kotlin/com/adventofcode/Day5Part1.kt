...
class Day5Part1 {
    ...
    data class Column(val points: List<Point>) : Iterable<Point> {
        fun numberOfDangerZones() = points.map(Point::notDiagonalIntersections).filter { it > 1 }.size
    ...
    data class Point(val lines: MutableList<Line> = mutableListOf()) {
        ...
        fun notDiagonalIntersections() = lines.filter(Line::notDiagonal).size
        fun render(): String {
            val size = lines.size
    ...
    data class Line(val start: Coordinate, val end: Coordinate) : Iterable<Coordinate> {
        ...
        fun notDiagonal() = vertical || horizontal
